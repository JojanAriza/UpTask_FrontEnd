...
export default function CreateProjectView() {
    ...
    const {mutate} =  useMutation({
        mutationFn: createProject, //no se le pasa parametros con react-query,
        onError: (error)=>{
    ...
    const handlesform =  (formData : ProjectFormData)=>{
        // const data = await createProject(formData)
        // toast.success(data)
        // navigate('/')
        // await mutation.mutateAsync(formData) //los datos que pasabamos en createproject no se pasan ahora en esa funcion si no en la de mutate
        mutate(formData) //las funciones asincronas las maneja reactquery
    }
    ...
    return (
        <>
            ...
            <form className="mt-10 bg-white shadow-lg p-10 rounded-lg"
                onSubmit={handleSubmit(handlesform)}
                noValidate
            >
